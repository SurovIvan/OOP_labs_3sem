...
void movement_thread_func(std::vector<std::unique_ptr<NPC>>& npcs, int map_width, int map_height) {
    ...
    while (true) {
        ...
        for (auto& npc : npcs) {
            if (npc && npc->isAlive()) {
                ...
                if (dynamic_cast<Dragon*>(npc.get())) {
                    move_range = 50;  
                } else if (dynamic_cast<Bull*>(npc.get())) {
                    move_range = 30;  
                } else if (dynamic_cast<Toad*>(npc.get())) {
                    move_range = 1;  
                }

                int new_x = npc->getX() + dis(gen) * move_range;
                int new_y = npc->getY() + dis(gen) * move_range;
        ...
        for (size_t i = 0; i < npcs.size(); ++i) {
            ...
            for (size_t j = i + 1; j < npcs.size(); ++j) {
                ...
                int killing_range = 0;
                if (dynamic_cast<Dragon*>(npcs[i].get()) || dynamic_cast<Dragon*>(npcs[j].get())) {
                    killing_range = 30;  
                } else if (dynamic_cast<Bull*>(npcs[i].get()) || dynamic_cast<Bull*>(npcs[j].get())) {
                    killing_range = 10;  
                } else if (dynamic_cast<Toad*>(npcs[i].get()) || dynamic_cast<Toad*>(npcs[j].get())) {
                    killing_range = 10;  
                }
...
int main() {
    ...
    NPCFactory::registerFactory("Dragon", std::unique_ptr<NPCFactory>(new DragonFactory()));
    NPCFactory::registerFactory("Toad", std::unique_ptr<NPCFactory>(new ToadFactory()));
    NPCFactory::registerFactory("Bull", std::unique_ptr<NPCFactory>(new BullFactory()));
    std::cout << "kfkf";
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        switch (type_rand) {
            ...
            case 1: type = "Toad"; break;
            case 2: type = "Bull"; break;
        }
    ...
    std::thread movement_thread(movement_thread_func, std::ref(npcs), MAP_WIDTH, MAP_HEIGHT);
